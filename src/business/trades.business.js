  ...
  if (body?.status != 'pending') {
    ...
    if (current_percentage_funds) {
      ...
      if (user?.funds > 0) {
        ...
        var script = all_active_trade
          .filter((script) => script.segment.includes('mcx'))
          .map((trade) => trade.script);
        var eqscript = all_active_trade
          .filter((script) => script.segment.includes('eq'))
          .map((trade) => trade.script);
        var mcx_scripts = script;
        var eq_scripts = eqscript;
        // function emitOnBothSockets(event, data) {
        //   return Promise.all([
        //     ...mcx_scripts.map((script) => {
        //       new Promise((resolve, reject) => {
        //         socket.emit(event, script);
        //       });
        //     }),
        //     ...eq_scripts.map((script) => {
        //       new Promise((resolve, reject) => {
        //         socket2.emit(event, script);
        //       });
        //     })
        //   ]);
        // }
    ...
    if (!body?.isCancel) {
      ...
      all_active_trades = await getActivetrades(body?.user_id);
      var pendimcx_scripts = pendingdata
        .filter((script) => script.segment.includes('mcx'))
        .map((trade) => trade.script);
      var pendieq_scripts = pendingdata
        .filter((script) => script.segment.includes('eq'))
        .map((trade) => trade.script);

      for (const script of [...pendimcx_scripts, ...pendieq_scripts]) {
        socket.emit('join', script);
      }
